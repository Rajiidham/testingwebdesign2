const subHeaders = [
    ...
    gsap.to('.container', { backgroundColor: '#e3e3e3', duration: 0.5 }); gsap.to('.placeholder, nav, footer, p', { color: '#000', duration: 0.5 });
    }
    items.forEach((item) => {
    ...
    function animateBlurEffect() {
        const letter = placeholder.children;
        let  index = 0;

        function clearNextLetter() {
            if (index < wrapLetters.length) {
